export  const products = [
    {
        image: "https://d2d22nphq0yz8t.cloudfront.net/88e6cc4b-eaa1-4053-af65-563d88ba8b26/https://media.croma.com/image/upload/v1645512514/Croma%20Assets/Communication/Mobiles/Images/248899_10_q19i23.png/mxw_2048,f_auto",
        name: "Samsung Galaxy S22",
        ourprice: "59,000.00",
        mrp: "74,999.00",
        off: "(20% off)",
        id: "1",
        no:"0"
    },
    {
        image: "https://d2d22nphq0yz8t.cloudfront.net/88e6cc4b-eaa1-4053-af65-563d88ba8b26/https://media.croma.com/image/upload/v1631796370/Croma%20Assets/Communication/Mobiles/Images/243510_7_mwm4qp.png/mxw_2048,f_auto",
        name: "iPhone 13 pro",
        ourprice: "1,05,000",
        mrp: "1,10,000",
        off: "(20% off)",
        id: "2",
        no:"0"
    },
    {
        image: "https://cdn1.smartprix.com/rx-iRp9KQOEU-w1200-h1200/nothing-phone-1-8gb.webp",
        name: "Nothing Phone1",
        ourprice: "35,499",
        mrp: "37,970",
        off: "(20% off)",
        id: "3",
        no:"0"
    },
    {
        image: "https://m.media-amazon.com/images/I/71b5BwTCcZL._SL1500_.jpg",
        name: "Samsung Galaxy M52",
        ourprice: "21,499",
        mrp: "22,999",
        off: "(20% off)",
        id: "4",
        no:"0"
    },
    {
        image: "https://encrypted-tbn0.gstatic.com/shopping?q=tbn:ANd9GcRaaBKGT6gKLSyhSwN0qzlzhlaXCzkF335SJxHi1gFZYew_hDpCgfx-FomXgWzIrJlfCTlzojnJMII&usqp=CAc",
        name: "IQOO Z6",
        ourprice: "12599",
        mrp: "15,499",
        off: "(20% off)",
        id: "5",
        no:"0"
    },

    {
        image: "https://motorolain.vtexassets.com/arquivos/ids/157173-800-800?v=637867665378900000&width=800&height=800&aspect=true",
        name: "Moto G52",
        ourprice: "13,800",
        mrp: "16,000",
        off: "(20% off)",
        id: "6",
        no:"0"
    }
];
